...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  const rows = await query('SELECT * FROM appliances WHERE id = $1', [params.id])
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await query(
    ...
    [name, brand, category, price, stock_count, description, JSON.stringify(photos), is_available, params.id]
  )
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await query('DELETE FROM appliances WHERE id = $1', [params.id])
