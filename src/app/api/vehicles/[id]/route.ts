...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  const vehicles = await query('SELECT * FROM vehicles WHERE id = $1', [params.id])
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await query(
    ...
    [name, brand, model, year, km, fuel, transmission, type, price, monthly_price, badge, description, JSON.stringify(photos), is_available, params.id]
  )
