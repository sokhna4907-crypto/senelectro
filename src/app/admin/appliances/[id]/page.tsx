...
export default function EditAppliance({ params }: { params: Promise<{ id: string }> }) {
      ...
      fetch(`/api/appliances/${p.id}`)
        ...
        .then(data => {
          ...
          if (a) setForm({
            ...
            category: a.category || 'television',
            price: a.price?.toString() || '',
            stock_count: a.stock_count?.toString() || '',
  ...
  const handleSave = async () => {
    ...
    setSaving(false)
    setSaved(true)
    setTimeout(() => router.push('/admin/appliances'), 1000)
  }
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center" style={{ background: '#F4F5F7' }}><div className="text-sm" style={{ color: '#aaa' }}>Chargement...</div></div>
      ...
      <main className="flex-1" style={{ background: '#F4F5F7' }}>
        <div className="flex items-center justify-between px-8 h-14 bg-white" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSave} disabled={saving} className="text-white text-xs px-5 py-2"
            ...
            <div className="grid grid-cols-2 gap-5">
              <Field label="Nom de l'appareil">
                <input value={form.name} onChange={e => update('name', e.target.value)} autoComplete="off"
                  placeholder="Télévision 55 pouces" className="w-full px-3 py-2 text-sm" style={inputStyle} />
              </Field>
              <Field label="Marque">
                <input value={form.brand} onChange={e => update('brand', e.target.value)} autoComplete="off"
                  placeholder="Samsung, LG, Sony..." className="w-full px-3 py-2 text-sm" style={inputStyle} />
              </Field>
              ...
              <Field label="Stock restant">
                <input type="number" value={form.stock_count} onChange={e => update('stock_count', e.target.value)} autoComplete="off"
                  placeholder="5" className="w-full px-3 py-2 text-sm" style={inputStyle} />
              </Field>
            ...
            <div className="mt-5">
              <Field label="Description">
                <textarea value={form.description} onChange={e => update('description', e.target.value)}
                  rows={3} className="w-full px-3 py-2 text-sm resize-none" style={inputStyle} />
              </Field>
            </div>
          </div>
        </div>
