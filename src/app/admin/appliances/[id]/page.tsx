...
export default function EditAppliance({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(async p => {
      ...
      try {
        const res = await fetch(`/api/appliances/${p.id}`)
        ...
        if (a) {
          ...
          setTimeout(() => {
            ...
            if (brandRef.current) brandRef.current.value = a.brand || ''
            if (priceRef.current) priceRef.current.value = a.price?.toString() || ''
            if (stockRef.current) stockRef.current.value = a.stock_count?.toString() || ''
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, brand, category, price: Number(price), stock_count: Number(stock), description: descRef.current?.value || '', photos, is_available: true }) })
      ...
      <main className="flex-1" style={{ background: '#F4F5F7' }}>
        <div className="flex items-center justify-between px-8 h-14 bg-white" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>
          <div className="flex items-center gap-3 text-sm" style={{ color: '#08111F' }}>
            <Link href="/admin/appliances" style={{ color: '#aaa' }}>Électroménager</Link><span style={{ color: '#ddd' }}>/</span><span className="font-medium">{id === 'new' ? 'Ajouter' : 'Modifier'}</span>
          </div>
        ...
        <div className="p-8 max-w-2xl">
          ...
          <div className="bg-white rounded p-6 mb-5" style={{ border: '0.5px solid rgba(0,0,0,0.07)' }}>
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div><label className="block text-xs uppercase tracking-widest mb-2" style={{ color: '#aaa' }}>Marque *</label><input ref={brandRef} type="text" placeholder="Samsung, LG..." style={inp} /></div>
