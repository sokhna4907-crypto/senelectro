...
async function getVehicles(): Promise<Vehicle[]> {
  try {
    return await query('SELECT * FROM vehicles WHERE is_available = true ORDER BY created_at DESC LIMIT 6')
  } catch { return [] }
}

async function getAppliances(): Promise<Appliance[]> {
  try {
    return await query('SELECT * FROM appliances WHERE is_available = true AND stock_count > 0 ORDER BY created_at DESC LIMIT 6')
  } catch { return [] }
}
    ...
    <main style={{ background: '#08111F' }}>
      ...
      <div className="px-4 md:px-12 py-6 flex items-center gap-0 overflow-x-auto" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
        ...
        <div className="flex flex-1">
          {brands.map((brand, i) => (
            <div key={brand} className="flex-1 text-center py-2 text-xs uppercase tracking-widest"
      ...
      {/* Véhicules */}
      <section className="px-4 md:px-12 py-12 md:py-20" style={{ background: '#F0F0F0' }}>
        <div className="flex items-end justify-between mb-14">
          <div>
            <div className="text-xs mb-4 uppercase tracking-widest" style={{ color: '#C08A45', letterSpacing: '3px' }}>Inventaire</div>
            <h2 className="text-2xl md:text-4xl font-light" style={{ color: '#08111F', letterSpacing: '-0.5px' }}>Véhicules en stock</h2>
          </div>
          <Link href="/vehicles" className="text-xs uppercase tracking-widest pb-1" style={{ color: '#C08A45', borderBottom: '0.5px solid rgba(192,138,69,0.3)', letterSpacing: '2px' }}>
            Tout voir →
          </Link>
        </div>
        {vehicles.length === 0 ? (
          <div className="text-center py-20" style={{ color: '#999' }}>Aucun véhicule disponible pour le moment</div>
        ) : (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4" style={{ background: '#C08A45' }}>
        {[
          { num: vehicles.length + '+', label: 'Véhicules en stock' },
          { num: '1 200+', label: 'Clients satisfaits' },
          { num: '10 min', label: 'Réponse rapide' },
      ...
      {/* Électroménager */}
      <section className="px-4 md:px-12 py-12 md:py-20" style={{ background: '#F0F0F0' }}>
        <div className="flex items-end justify-between mb-6 pb-8" style={{ borderBottom: '0.5px solid #D5D5D5' }}>
          <div>
        ...
        {appliances.length === 0 ? (
          <div className="text-center py-20" style={{ color: '#999' }}>Aucun appareil disponible pour le moment</div>
        ) : (
